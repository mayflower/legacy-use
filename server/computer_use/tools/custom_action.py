...
class CustomActionTool(BaseAnthropicTool):
    ...
    async def __call__(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not action_name:
                return ToolFailure(error='Missing required parameter: action_name')
            if not tool_collection:
                return ToolFailure(error='Missing required parameter: tool_collection')
            ...
            for tool_action in action['tools']:
                ...
                if result.error:
                    return ToolFailure(error=result.error)
