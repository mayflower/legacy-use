...
"""
    ...
    def _extract_function_parameters(
        self, func_call: str, positional_keys: Optional[list[str]] = None
    ) -> Dict[str, str]:
    ...
    def _convert_pyautogui_code_to_tool_use(self, code: str) -> BetaToolUseBlockParam:
        ...
        def _convert_coordinate(coordinate: str) -> tuple[int, int]:
            ...
            params = self._extract_function_parameters(coordinate, ['x', 'y'])
        ...
        if command.startswith('click'):
            ...
            params = self._extract_function_parameters(command, ['amount'])
            ...
            params = self._extract_function_parameters(command)
            text = params.get('message', '')
            return _construct_tool_use('type', text=text)
        elif command.startswith('press'):
            # press('esc') or press(key='esc')
            params = self._extract_function_parameters(command, ['key'])
            key = params.get('key', '')
            ...
            params = self._extract_function_parameters(command, ['keys'])
            ...
            params = self._extract_function_parameters(command, ['seconds'])
